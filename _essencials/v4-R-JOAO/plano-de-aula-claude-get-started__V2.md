...
> *» IA: bloco de CAPA (Cover). Use o título acima e o subtítulo abaixo, sem reescrever. Não gere conteúdo adicional aqui. O tema do curso é o do CONTEÚDO (Claude Desktop), nunca o destas anotações.*
...
> *» IA: bloco de LISTA. Objetivos mensuráveis, verbatim, sem acréscimos.*
...
> *» IA: bloco(s) de TEXTO. Pode dividir em blocos menores para leitura, mas mantenha o conteúdo e o sentido. Não resuma. Preserve termos de interface e caminhos.*
...
> *» IA: bloco STATEMENT. Caminho de navegação para conferir a arquitetura — preserve verbatim.*
>
> 🧭 **Caminho:** Windows — `Configurações > Sistema > Sobre > Tipo de sistema`. macOS — `menu Apple () > Sobre este Mac > Chip (ou Processador)`.
...
> *» IA: o título da atividade vira STATEMENT; os passos viram bloco PROCESSO ou CHECKLIST; o gabarito/critério, um STATEMENT. Mantenha procedimentos e respostas verbatim.*
...
> *» IA: bloco STATEMENT ou DOWNLOAD com o prompt-modelo. Mantenha o texto entre aspas VERBATIM — é um modelo a copiar. Não preencha os campos entre colchetes.*
...
> *» IA: bloco STATEMENT. Localização da interface — preserve verbatim.*
>
> 🧭 **Onde observar:** `Claude Desktop > aba Code > painel lateral (lista de arquivos, à esquerda) + área de conversa (ao centro)`.
...
> *» IA: bloco STATEMENT. Ponto de observação do ciclo iterativo — preserve verbatim.*
>
> 🧭 **Onde observar:** `Claude Desktop > aba Chat > histórico da conversa`, com ao menos dois turnos: solicitação inicial, resposta e reformulação refinada.
...
> *» IA: bloco STATEMENT. Localização da interface Cowork — preserve verbatim.*
>
> 🧭 **Onde observar:** `Claude Desktop > aba Cowork > painel de status da missão + relatório de entrega`.
...
> *» IA: bloco STATEMENT. Estrutura mínima do projeto — preserve verbatim.*
>
> 🧭 **Estrutura esperada na pasta-projeto:** `README.md`, `.gitignore` e ao menos um arquivo de conteúdo (ex.: `main.py`), visíveis em `Windows Explorer` ou `Finder`.
...
> *» IA: bloco STATEMENT (ponto de controle). Preserve verbatim.*
>
> 🏁 **Ponto de controle:** o projeto aparece carregado em `Claude Desktop > aba Code > painel lateral`, confirmando a conclusão do exercício.
...
> *» IA: ▼▼▼ INÍCIO DA LIÇÃO — "Módulo 6" — todo o conteúdo até o ▲▲▲ FIM correspondente é UMA única lição no Rise; não dividir nem fundir, mesmo que haja títulos dentro de exemplos de código. ▼▼▼*
...
> *» IA: bloco STATEMENT. Localização e leitura da diff view — preserve verbatim.*
>
> 🧭 **Onde observar:** `Claude Desktop > aba Code > diff view` — linhas removidas em vermelho, linhas adicionadas em verde.
...
> *» IA: ▼▼▼ INÍCIO DA LIÇÃO — "Módulo 7" — todo o conteúdo até o ▲▲▲ FIM correspondente é UMA única lição no Rise; não dividir nem fundir, mesmo que haja títulos dentro de exemplos de código. ▼▼▼*
...
> *» IA: bloco STATEMENT (ponto de controle). Preserve verbatim.*
>
> 🏁 **Ponto de controle:** ao final, o comando `git log` no terminal exibe ao menos dois commits registrados na sessão — evidência do fluxo completo de trabalho.
...
> *» IA: ▲▲▲ FIM DA LIÇÃO — "Módulo 7" — encerre esta lição aqui; o que vier a seguir é outra lição. ▲▲▲*
